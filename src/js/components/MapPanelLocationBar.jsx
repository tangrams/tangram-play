...
export default class MapPanelLocationBar extends React.Component {
  ...
  unthrottledReverseGeocode(latlng) {
    ...
    const endpoint = `https://${config.SEARCH.HOST}/v1/reverse?point.lat=${lat}&point.lon=${lng}&size=1&layers=coarse&api_key=${config.SEARCH.API_KEY}`;
  ...
  autocomplete(query) {
    const center = map.getCenter();
    // eslint-disable-next-line max-len
    const endpoint = `https://${config.SEARCH.HOST}/v1/autocomplete?text=${query}&focus.point.lat=${center.lat}&focus.point.lon=${center.lng}&layers=coarse&api_key=${config.SEARCH.API_KEY}`;
  ...
  search(query) {
    const center = map.getCenter();
    // eslint-disable-next-line max-len
    const endpoint = `https://${config.SEARCH.HOST}/v1/search?text=${query}&focus.point.lat=${center.lat}&focus.point.lon=${center.lng}&layers=coarse&api_key=${config.SEARCH.API_KEY}`;
